...
local function CreateAdminUI()
        ...
        b.MouseButton1Click:Connect(function()
            ...
            if requiresTarget and not selectedCmdTarget then
                StarterGui:SetCore("ChatMakeSystemMessage", {
                    Text = "[Lcc Hub] ❌ Selecione um jogador na aba de Comandos!",
            ...
            pcall(function()
                ...
                if channel then
                    ...
                    StarterGui:SetCore("ChatMakeSystemMessage", {
                        Text = "[Lcc Hub] Erro: Canal de chat não encontrado!",
                        Color = Color3.fromRGB(255, 100, 100),
                        Font = Enum.Font.GothamBold
                    })
                end
    ...
    btnChatAs.MouseButton1Click:Connect(function()
        ...
        if not selectedCmdTarget then
            StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = "[Lcc Hub] ❌ Selecione um jogador alvo primeiro!",
                Color = Color3.fromRGB(255, 100, 100),
                Font = Enum.Font.GothamBold
            })
            return
    ...
    btnCopyAvatar.MouseButton1Click:Connect(function()
        if not selectedCopier or not selectedAvatarTarget then
            StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = "[Lcc Hub] ❌ Selecione o Copiador e o Alvo na aba Avatar!",
                Color = Color3.fromRGB(255, 100, 100),
                Font = Enum.Font.GothamBold
            })
            return
    ...
    for _, jumpscare in ipairs(JUMPSCARES) do
        ...
        b.MouseButton1Click:Connect(function()
            if not selectedJumpscareTarget then
                StarterGui:SetCore("ChatMakeSystemMessage", {
                    Text = "[Lcc hub] ❌ Selecione um jogador na aba Comandos!",
                    Color = Color3.fromRGB(255, 100, 100),
                    Font = Enum.Font.GothamBold
                })
                return
...
local function ExecuteAvatarCopy(targetPlayerName, copierPlayerName)
    ...
    if not (targetPlayer and copierPlayer and copierPlayer.Character and targetPlayer.Character) then
        StarterGui:SetCore("ChatMakeSystemMessage", {
            Text = "[Lcc Hub] Não foi possível encontrar os jogadores para copiar o avatar.",
            Color = Color3.fromRGB(255, 100, 100),
            Font = Enum.Font.GothamBold
        })
        return false
...
local function analisarChatDe(chatter, message)
    ...
    if msg == ";verifique" then
        if not IsAutorizado(chatter.Name) then
            if chatter == LocalPlayer then
                StarterGui:SetCore("ChatMakeSystemMessage", {
                    Text = "[Lcc Hub] ❌ Você não tem permissão para usar ;verifique.",
                    Color = Color3.fromRGB(255, 100, 100),
                    Font = Enum.Font.GothamBold
                })
            end
    ...
    if msg == "Lcc_####" then
        ...
        StarterGui:SetCore("ChatMakeSystemMessage", {
            Text = "[Lcc Hub] " .. chatter.Name .. " identificado como Usuário Lcc",
            Color = Color3.fromRGB(255, 105, 180),
            Font = Enum.Font.GothamBold
        })
        return
    ...
    if not IsAutorizado(chatter.Name) then
        if chatter == LocalPlayer then
            StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = "[Lcc Hub] ❌ Você não tem permissão para usar este comando.",
                Color = Color3.fromRGB(255, 100, 100),
                Font = Enum.Font.GothamBold
            })
        end
    ...
    if args[1] == ";ch" and #args >= 3 then
        ...
        elseif chatter == LocalPlayer then
            StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = "[Lcc Hub] ❌ Jogador alvo não encontrado ou mensagem inválida.",
                Color = Color3.fromRGB(255, 100, 100),
                Font = Enum.Font.GothamBold
            })
        end
    ...
    if args[1] == ";copy" and #args == 3 then
        ...
        elseif chatter == LocalPlayer then
            StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = "[Lcc Hub] ❌ Jogador alvo ou copiador não encontrado.",
                Color = Color3.fromRGB(255, 100, 100),
                Font = Enum.Font.GothamBold
            })
        end
    ...
    for _, jumpscare in ipairs(JUMPSCARES) do
        if args[1] == jumpscare.Name and #args == 2 then
            ...
            elseif chatter == LocalPlayer then
                StarterGui:SetCore("ChatMakeSystemMessage", {
                    Text = "[Lcc Hub] ❌ Jogador alvo não encontrado.",
                    Color = Color3.fromRGB(255, 100, 100),
                    Font = Enum.Font.GothamBold
                })
            end
    ...
    if table.find(targetCommands, args[1]) and #args == 2 then
        ...
        elseif args[1] == ";kick" then
            if targetPlayer == LocalPlayer and IsDono(LocalPlayer.Name) and not IsDono(chatter.Name) then
                if chatter == LocalPlayer then
                    StarterGui:SetCore("ChatMakeSystemMessage", {
                        Text = "[Lcc Hub] ❌ Você não pode expulsar um dono.",
                        Color = Color3.fromRGB(255, 100, 100),
                        Font = Enum.Font.GothamBold
                    })
                end
            ...
            targetPlayer:Kick("Você foi removido pelos seus atos --equipe Lcc hub")
            success = true
        ...
        elseif chatter == LocalPlayer then
            StarterGui:SetCore("ChatMakeSystemMessage", {
                Text = "[Lcc Hub] ❌ Falha ao executar " .. args[1] .. " em " .. targetPlayer.Name .. ". Verifique se o jogador está no jogo.",
                Color = Color3.fromRGB(255, 100, 100),
                Font = Enum.Font.GothamBold
            })
        end
                ...
                if root then
                    ...
                    if d <= 25 and not tags[p] then
                        ...
                        if IsDono(p.Name) then
                            tag = CriarTag("Dono Lcc", Color3.fromRGB(255, 0, 0), Color3.fromRGBColor3(255, 60, 60))
                        elseif IsMod(p.Name) or IsTempMod(p.Name) then
                            tag = CriarTag("Moderador Lcc hub", Color3.fromRGBColor3(0, 191, 255), Color3.fromRGB(255, 105, 180))
                        elseif Lcc_USERS[p.Name] then
                            tag = CriarTag("Usuário Lcc", Color3.new(1, 1, 1), Color3.new(0.78, 0.78, 0.78))
                        end
                        if tag then
                            tag.Adornee = head
